...
class API extends Controller{
    ...
    function importExcel(){
        //  Include thư viện PHPExcel_IOFactory vào
        require_once './public/thuvien/PHPExcel/Classes/PHPExcel/IOFactory.php';
        $inputFileName = './public/backup/excel/phimle.xlsx';
        ...
        foreach ($rowData as $key => $value) {
            $value=$value[0];
            $link=str_replace("openCity(event,'","",$value[1]);
            $link=str_replace("')","",$link);
            $idPhim=$this->model('PhimModel')->getSlugPhim($value[5]);

            $this->model('LinkPhimModel')->themLink1($value[0],$idPhim,$value[2],$link,$value[4],$value[3]);
        }
    }
    function XuatSQL()
    {
